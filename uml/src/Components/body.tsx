...
const DiagramComponent: React.FC = () => {
  ...
  const handleModelChange = (changes: go.IncrementalData) => {
    ...
    setState({
      nodeDataArray: diagramRef.current.model.nodeDataArray as DiagramNodeData[],
      linkDataArray: (diagramRef.current.model as go.GraphLinksModel).linkDataArray as DiagramLinkData[]
    });
  };

  return (
    <div className="diagram-wrapper" style={{ width: '100%', height: '500px', border: '1px solid black' }}>
      <ReactDiagram
